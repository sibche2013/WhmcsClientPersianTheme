...
$pdf->Cell(0, 6, Lang::trans('invoicesdatecreated') . ': ('.jdate("Y/m/d", strtotime($datecreated)).') ' . $datecreated, 0, 1, 'R', '1');
$pdf->Cell(0, 6, Lang::trans('invoicesdatedue') . ': ('.jdate("Y/m/d", strtotime($duedate)).') ' . $duedate, 0, 1, 'R', '1');
$pdf->Ln(10);
...
foreach ($invoiceitems as $item) {
    $tblhtml .= '
    <tr bgcolor="#fff">
        <td align="right">' . nl2br($item['description']) . '</td>
...
if (!count($transactions)) {
    ...
    foreach ($transactions AS $trans) {
        $tblhtml .= '
        <tr bgcolor="#fff">
            <td align="center">' . $trans['date'] .' ('.jdate("Y/m/d", strtotime($invoicestransdate)).')</td>
...
$pdf->SetFont($pdfFont, '', 8);
$pdf->Ln(5);
$pdf->Cell(180, 4, Lang::trans('invoicepdfgenerated') . ' ' .jdate('Y/n/j'), '', '', 'C');
